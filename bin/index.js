...
class Alphabet {
    constructor(path_to_alphabet_file) {
        this.alphabetWithFrequency = Object.entries(JSON.parse(fs.readFileSync('russian_alphabet.json', 'utf8')))
            .map(([letter, frecuency]) => ({ letter: letter, frecuency: frecuency }));
    }
}
function main() {
    ...
    let russian_alphabet = new Alphabet("russian_alphabet.json");
    console.log(`Русский алфавит с частотностью: \n${russian_alphabet.alphabetWithFrequency.map(e => `${e.letter} : ${e.frecuency}\n`)}`);
    let russian_encrypt_text = caesar_algorithm(textObj.text, textObj.key, russian_alphabet, true);
    console.log(`Зашифрованный текст: ${russian_encrypt_text}`);
    let russian_decrypted_by_brute_force = decrypt_by_brute_force(russian_encrypt_text, russian_alphabet);
    ...
    let english_encrypt_text = caesar_algorithm(EnglishTextObject.text, EnglishTextObject.key, english_alphabet, true);
    console.log(`Зашифрованный текст: ${english_encrypt_text.slice(0, TEXT_PREVIEW_MAX_SIZE)}...`);
    let english_decrypted_by_brute_force = decrypt_by_frequency_analysis(english_encrypt_text, english_alphabet);
...
function caesar_algorithm(text, key, alphabet, encrypt) {
    let encrypt_letter = "";
    let result_text = "";
...
function decrypt_by_brute_force(encrypt_text, alphabet) {
    let decrypted_text = "";
    let letters = alphabet.alphabetWithFrequency.map((e) => e.letter);
    for (let key_ = 0; key_ < letters.length; key_++) {
        decrypted_text = caesar_algorithm(encrypt_text, key_, alphabet, false);
        if (_is_decrypted_text(decrypted_text, alphabet)) {
            return decrypted_text;
        }
    }
    return " не удалось расшифровать ";
}
function _is_decrypted_text(text, alphabet) {
    ...
    const text_frequency = alphabet.alphabetWithFrequency.map((e) => {
        return {
            ...
            frecuency: (() => {
                let count = 0;
                for (let i = 0; i < text.length; i++) {
                    if (text[i].toLowerCase() === e.letter)
                        count++;
                }
    ...
    for (let i = 0; i < text_frequency.length; i++) {
        total_difference += Math.pow((alphabet_frequency[i] - text_frequency[i].frecuency), 2);
    }
    return total_difference <= CONFIDENCE_VALUE;
}
function decrypt_by_frequency_analysis(encrypt_text, alphabet) {
    ...
    let most_popular_letter_in_alphabet = alphabet.alphabetWithFrequency.find(e => e.frecuency === Math.max(...alphabet_frequency));
    let most_popular_letter_in_text = text_frequency.find(e => e.frecuency === Math.max(...text_frequency.map(e => e.frecuency)));
    let index_from_alphabet = alphabet.alphabetWithFrequency.findIndex(e => e.letter === most_popular_letter_in_text.letter);
    let index_from_text = alphabet.alphabetWithFrequency.findIndex(e => e.letter === most_popular_letter_in_text.letter);
...
function _shift_letter(letter, key, alphabet, encrypt) {
    let shifted_index;
